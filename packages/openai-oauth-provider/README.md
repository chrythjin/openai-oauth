...
| Codex API version | `codexVersion` | Local `codex --version`, then `@openai/codex` latest from npm, then `0.124.0` | Override the Codex API client version used for model discovery. This mainly matters for CLI-style model catalog lookups, but the provider accepts it for config parity. |
...
- Streaming text
- Tool calls
- Responses-style models such as `gpt-5.5`
