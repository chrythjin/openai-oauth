...
const createCliParser = (argv: string[]) =>
	yargs(argv)
		...
		.option("dashboard-dist", {
			type: "string",
			describe: "Path to the dashboard dist directory (enables the web UI).",
		})
...
export const parseCliArgs = (argv: string[]): CliArgs => {
	...
	return {
		...
		dashboardDistPath: parsed.dashboardDist,
	}
}
...
export const toServerOptions = (args: CliArgs) => ({
	...
	dashboardDistPath: args.dashboardDistPath,
})
...
export const runCli = async (argv: string[] = hideBin(process.argv)) => {
	...
	let availableModels = options.models ?? ["(model discovery unavailable)"]
	try {
		availableModels = await resolveOpenAIOAuthModels(client, options.models, {
		...
		const reason =
			error instanceof Error ? error.message : "Unknown model discovery error."
		console.error(`Failed to auto-discover models: ${reason}`)
		console.error(
			"Server is running, but upstream requests may fail until you refresh auth with `npx @openai/codex login`.",
		)
	}
