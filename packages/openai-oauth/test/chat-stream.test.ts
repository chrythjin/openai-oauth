...
async function collectSseData(response: Response): Promise<SseChunk[]> {
...
function findToolCallChunks(chunks: SseChunk[]): SseChunk[] {
	return chunks.filter((chunk) => chunk.choices?.[0]?.delta?.tool_calls != null)
}

const dummyProvider = vi.fn(() => ({})) as unknown as OpenAIOAuthProvider & {
	mockClear: () => void
}
...
describe("streamChatCompletions", () => {
	test("maps model aliases to upstream reasoning effort", async () => {
		dummyProvider.mockClear()
		...
		expect(dummyProvider).toHaveBeenCalledWith("gpt-5.4-mini")
		...
		expect(chunks[0]).toMatchObject({ model: "gpt-5.4-mini-xhigh" })
	})
	...
	test("emits tool call arguments from tool-call event when model skips deltas", async () => {
		...
		if (args == null) {
			throw new Error("Expected streamed tool-call arguments.")
		}
