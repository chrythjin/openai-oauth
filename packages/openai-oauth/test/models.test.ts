...
describe("model discovery", () => {
	...
	test("uses a pinned fallback when local and remote version discovery fail", async () => {
		...
		expect(warnings).toEqual([
			"Could not determine the Codex API version automatically. Falling back to 0.124.0. Pass a version explicitly with --codex-version if you need to override it.",
		])
	})
